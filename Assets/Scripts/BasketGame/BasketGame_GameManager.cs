...
//		fruitTag
	...
	public void HangFruitOnTree(){
		...
		float max_width_percentage = .2f, min_width_percentage = 0.05f;
		...
		foreach (var basket in all_baskets) {
			...
			for (int i = 0; i < basket_capacity; i++) {
				...
				var fruitName = Camera.main.GetComponent<BasketGame_SceneVariables>().GetColoredFruit (basket.GetComponent<BasketBehavior>().basketName);
